...
Airport::Airport
	...
	: Tile(tile_cost, name, owner)
	, tile_level_(0)
	, rent_(rent)
{
}

int Airport::GetTileLevel() const
{
	return tile_level_;
}

std::array<int, 4> Airport::GetRent() const
{
	return rent_;
}

void Airport::SetTileLevel(const int tile_level)
{
	tile_level_ = tile_level;
}

void Airport::Reset(const std::string &bank_name)
{
	owner_ = bank_name;
	tile_level_ = 0;
}
...
// cod "furat" de la railroad.cpp si absolut netestat
void Airport::Print(std::ostream &os) const
{
